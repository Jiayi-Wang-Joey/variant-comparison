...
add_tech_all <- \(dt) {
    ...
    dt[, cell_line := sub("^([^-]+)-.*$", "\\1", sample)]
    dt[cell_line == "HG002a", cell_line := "HG002"]
    dt <- dt[cell_line %in% c("HG002", "HG004", "HG005")]
    dt[, cell_line := factor(cell_line, levels = c("HG002", "HG004", "HG005"))]
    dt[, tech := ifelse(grepl("MasSeq|IsoSeq", tech),
                         paste0("<span style='color:#54278f;'>", tech, "</span>"),
                         paste0("<span style='color:#d95f0e;'>", tech, "</span>"))]
    dt
...
# ---- inputs re-derived across all cell lines (a/b above are HG002-only) ----
redi_all_files <- args[[1]][!grepl("/spectrum/", args[[1]])]
redi_all <- rbindlist(lapply(redi_all_files, fread))
redi_all <- redi_all[Type == "SNP" & tool != "GATK"]
redi_all[, coverage := as.integer(coverage)]
redi_all <- add_tech_all(redi_all)

# NOTE: not produced by any Snakemake rule (untracked, manually cached RDS) -
# regenerate by hand if results/happy/ changes.
happy_all <- as.data.table(readRDS("data/results/happy.csv"))
happy_all <- happy_all[Filter == "PASS" & Type == "SNP" & tool != "GATK"]
happy_all[, coverage := as.integer(as.character(coverage))]
happy_all <- add_tech_all(happy_all)

# --- c: total FP (all substitution types), stacked (REDIportal-confirmed vs other), coverage=5, all cell lines ---
redi_agg_all <- redi_all[coverage == REF_COV, .(FP = sum(FP), FP_redi_strict = sum(FP_in_REDIportal_strict)),
                          by = .(tool, cell_line, tech)]
redi_agg_all[, FP_other := FP - FP_redi_strict]
redi_agg_all[, pct := round(100 * FP_redi_strict / FP, 1)]

long_c <- melt(redi_agg_all, id.vars = c("tool", "cell_line", "tech", "pct", "FP"),
               measure.vars = c("FP_redi_strict", "FP_other"),
               variable.name = "category", value.name = "count")
long_c[, category := factor(category, levels = c("FP_other", "FP_redi_strict"),
                             labels = c("Other FP", "RNA editing (REDIportal)"))]

pc <- ggplot(long_c, aes(tool, count, fill = category)) +
    geom_bar(stat = "identity") +
    geom_text(data = redi_agg_all, aes(x = tool, y = FP, label = paste0(pct, "%")),
              inherit.aes = FALSE, vjust = -0.3, size = 2) +
...
# --- d: current (original) F1 after REDIportal tagging, vs coverage, all cell lines ---
dd <- happy_all[, .(TP = sum(TRUTH.TP), FP = sum(QUERY.FP), FN = sum(TRUTH.FN)),
                by = .(tool, cell_line, tech, coverage)]
dd[, F1 := 2 * TP / (2 * TP + FP + FN)]
dd[, coverage := factor(coverage, levels = sort(unique(coverage)))]

pd <- ggplot(dd, aes(coverage, F1, color = tool, group = tool)) +
    geom_point(size = 1.5, alpha = 0.8) +
    geom_line(linewidth = 0.8, alpha = 0.8) +
    facet_grid2(cell_line ~ tech, scales = "free") +
    scale_color_manual(values = tool_cols) +
    common_theme +
    theme(axis.text.x = element_text(size = 7, angle = 0, hjust = 0.5)) +
    labs(x = "Coverage Cutoff (DP >= n)", y = "F1 after REDIportal tagging", color = "Variant Caller")

top_group <- wrap_plots(pa, pc, ncol = 1, heights = c(1, 3)) +
    plot_layout(guides = "collect") &
    theme(legend.position = "bottom")

bottom_group <- wrap_plots(pb, pd, ncol = 1, heights = c(1, 3)) +
    plot_layout(guides = "collect") &
    theme(legend.position = "bottom")

gg <- wrap_plots(top_group, bottom_group, ncol = 1, heights = c(4, 4)) +
    plot_annotation(tag_levels = "a") &
    theme(plot.tag = element_text(face = "bold"))

ggsave(args[[2]], gg, width = 27, height = 38, units = "cm")
