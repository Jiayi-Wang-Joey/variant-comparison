...
.p <- \(mmp, pbm, title, variant_type = "SNP", legend = FALSE) {
...
sample <- c(
    "HG002a-MasSeq", "HG002a-Baylor-IsoSeq",
    "HG004-MasSeq", "HG004-Baylor-IsoSeq",
    "HG005-MasSeq", "HG005-Baylor-IsoSeq"
)
excluded_sample <- "HG004-Baylor-IsoSeq"
...
.mkrow <- \(s, legend = FALSE) {
    ps <- lapply(seq_along(caller), \(i) {
        ...
        .p(mmp, pbm, title = c, variant_type = "SNP", legend = legend && i == 1)
    })
    label <- wrap_elements(grid::textGrob(
        sub("^HG002a", "HG002", s), rot = 90, gp = grid::gpar(fontsize = 12, fontface = "bold")
    ))
    cat("done:", s, "\n")
...
out <- "plts/upset_FPs_all-datasets.pdf"
